...
/*:
 - experiment:(実験): 上のコードで`verseOne`関数を編集して`merrilyDream()`を呼び出した後に`versaOne()`を呼び出すようにし、無限ループを作ります。コンソールと結果サイドバーを見てください。行を削除してループを止めます。Playgroundが元に戻るまでにしばらく時間がかかるかもしれません。無限ループは大変な仕事なのです。
 */
/*: 
