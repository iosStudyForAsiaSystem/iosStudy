 ...
 */
/*:  
>(注意): `Double`型という名前は「Double-precision floating point」（倍精度浮動小数点）を参照することに由来します。`Float`型も小数点を含む数を参照しますが、デフォルトの`Double`の方が倍の精度を持ちます。
*/
let partNumber = 3.2
let wholeNumber = 2
//: - experiment:(実験): `partNumber`（小数点を含む数字）と`wholeNumber`（整数）を使って、2つを加算するなどの計算を実行してみてください。エラーを確認します。両方が整数、または両方が小数になるように値を変更し、どのように違うか確認します。
