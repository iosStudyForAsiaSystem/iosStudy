 ...
 */
/*:
- experiment:(実験): 下の各コード行をコメント解除し、生成されるエラーを確認してください。Playgroundはエラーを見つけるとそこで止まってしまうので、1行ずつコメント解除する必要があります。
*/
//"banana" + 1
//2 + "2"
/*:
 ...
 */
/*:  
 - `Binary operator '+'`（2項演算子「`+`」）：（すでに学習した）加算演算子`+`を、2項演算子と呼びます。2項演算子は、その左右に何かが入ります。
     
     
 - `cannot be applied to operands`：_Operands_（オペランド）とは、「banana」という文字列や「2」という整数のように、演算子が扱う対象となるものです。
 ...
 */
/*: 
- callout(演習): 間違った加算のステートメントをもう少し記述してみます。各行を記述する前に、どのようなエラーが生成されるか予想してください。先ほどの実験のコード行からコメント解除してください。
*/
